...
1. What package manager(s) and runtime versions are canonical?
   (e.g., uv, npm, cargo — and what commands are forbidden?)
1. What is the branching/commit strategy?
   (e.g., conventional commits, squash merge, required trailers?)
1. What is the quality gate? List every command the agent MUST run before
   committing (lint, test, build, type-check — with exact commands).
1. Is there a spec/doc-first workflow (e.g., SDD, ADR, tickets)?
   If so, describe the phases and required file paths.
1. What are the hard file-system rules?
   (e.g., temp files under `./work/tmp/`, never `/tmp/`)
1. What naming conventions must be enforced?
   (files, functions, classes, constants — per language)
1. Are there multiple AI models in use? Which model handles which phase?
   (e.g., Opus for design, Sonnet for implementation)
1. What state-tracking mechanism should agents use between sessions?
...
1. **Session Start** — how agent resumes state (STATE.md schema if applicable)
1. **Workflow** — the exact phase sequence with commit message formats
1. **Quality Gate** — runnable shell block + manual checklist (include STATE.md update step)
1. **Environment Rules** — commands table (do/don't), package manager rules
1. **File System Rules** — temp dirs, project tree conventions
1. **Code Standards** — naming, typing, DRY, scan-before-coding rule
1. **Documentation Standards** — markdown lint, file naming, changelog rules
1. **Communication Protocol** — phase announcements, approval gates, evidence reporting
1. **Model Selection** _(if applicable)_ — phase → model mapping
